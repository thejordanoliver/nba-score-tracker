...
export default function LoginScreen() {
  ...
  const handleSignupSubmit = async (): Promise<User | null> => {
    const {
      fullName,
      username,
      email,
  ...
  const handleSignup = async () => {
    const user = await handleSignupSubmit();
    if (!user) return;

    try {
      const res = await axios.post<User>(`${BASE_URL}/api/login`, {
        username: user.username,
        password: signupData.password, // use the password the user just entered
      });

      const loggedInUser = res.data;

      await safeSetItem("userId", loggedInUser.id?.toString() || null);
      await safeSetItem("username", loggedInUser.username);
      await safeSetItem("fullName", loggedInUser.full_name);
      await safeSetItem("email", loggedInUser.email);
      await safeSetItem(
        "profileImage",
        loggedInUser.profile_image
          ? `${BASE_URL}${loggedInUser.profile_image}`
          : null
      );
      await safeSetItem(
        "bannerImage",
        loggedInUser.banner_image
          ? `${BASE_URL}${loggedInUser.banner_image}`
          : null
      );
      await safeSetItem(
        "favorites",
        JSON.stringify(loggedInUser.favorites || [])
      );
      await safeSetItem("bio", loggedInUser.bio ?? "");
