...
const handleLogin = async () => {
  ...
  try {
    ...
    await safeSetItem("profileImage", user.profile_image ? `${BASE_URL}${user.profile_image}` : null);
    await safeSetItem("bannerImage", user.banner_image ? `${BASE_URL}${user.banner_image}` : null);
  ...
  const handleSignupSubmit = async (): Promise<{ user: User; token: string } | null> => {
    const {
      fullName,
      username,
      email,
    ...
    if (!fullName || !username || !email || password !== confirmPassword) {
      Alert.alert("Please check your details", "Make sure all fields are filled and passwords match.");
      return null;
    }
