...
function getSeriesKey(stage: number, team1Id: number, team2Id: number): string {
  const [low, high] = team1Id < team2Id ? [team1Id, team2Id] : [team2Id, team1Id];
  ...
  useEffect(() => {
    async function fetchGames() {
      ...
      try {
        ...
        // Filter all playoff games (finished or scheduled) between the two teams in stage 3
        const filteredGames = allGamesRaw.filter(
          (game) =>
            game.stage === 3 &&
            ((game.teams.home?.id === team1Id && game.teams.visitors?.id === team2Id) ||
              (game.teams.home?.id === team2Id && game.teams.visitors?.id === team1Id)) &&
            // Only include games with valid scores or scheduled games (scheduled might have 0 scores)
            (typeof game.scores.home?.points === "number" || game.status.long === "Scheduled")
        );

        // Group games by series key (just one key expected)
        const gamesBySeries = new Map<SeriesKey, ApiGameRaw[]>();
        for (const game of filteredGames) {
          const key = getSeriesKey(game.stage, game.teams.home.id, game.teams.visitors.id);
...
export function usePlayoffSeries(games: Game[]) {
  // No change needed here, unless you want to summarize all series overall
  return games;
}
