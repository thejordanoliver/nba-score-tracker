...
const SEASON_STAGE_DATES: Record<string, {
  playoffStart: string;
  playoffEnd: string;
  finalsStart: string;
  finalsEnd: string;
...
const getSeasonStageDates = (season: string) => {
  return SEASON_STAGE_DATES[season] || {
    playoffStart: "2025-04-20",
    playoffEnd: "2025-06-04",
    finalsStart: "2025-06-05",
    finalsEnd: "2025-06-30",
...
const toEasternDateTime = (isoDate: string): Date => {
  const dt = DateTime.fromISO(isoDate, { zone: "utc" }).setZone("America/New_York");
  return dt.toJSDate();
};
...
export function useSeasonGames(season: string) {
  ...
  const refreshGames = async () => {
    try {
      ...
      const response = await axios.get<{ games: DBGame[] }>(
        `${BACKEND_BASE_URL}/api/nba-season-2024/seasons/${season}/games`
      );

      const dbGames = response.data.games;

      const { playoffStart, playoffEnd, finalsStart, finalsEnd } = getSeasonStageDates(season);
      ...
      const nowETMidnight = DateTime.now().setZone("America/New_York").startOf("day");
  ...
  return keep;
        ...
        .map((game) => {
          ...
          let stage = 1;
          if (gameDateMidnightET >= playoffStartDate && gameDateMidnightET <= playoffEndDate) {
            stage = 2;
          } else if (gameDateMidnightET >= finalsStartDate && gameDateMidnightET <= finalsEndDate) {
            stage = 3;
          }
