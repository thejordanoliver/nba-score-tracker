...
import { Image, Text, TextStyle, TouchableOpacity, useColorScheme, View } from "react-native";
...
function NFLGameCard({ game, isDark }: Props) {
  ...
  const status = useMemo(() => {
    ...
    const live =
      !["Not Started", "Finished", "Canceled", "Delayed", "Postponed", "Halftime"].includes(long);
    return {
  ...
  return (
    <TouchableOpacity
      activeOpacity={0.85}
      onPress={() =>
        router.push({ pathname: "/game/nfl/[game]", params: { game: JSON.stringify(game) } })
      }
    >
