...
export default function TeamCard({
  ...
  const backgroundColor = selectionAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [isDark ? "#222" : "#eee", item.color ?? "#000"],
  });
  ...
  useEffect(() => {
    if (!isDark && previousShouldShowLight.current !== shouldShowLight) {
    ...
    >
      ...
      >
        ...
        {isGridView ? (
          ...
          <Animated.Text style={[styles.teamName, { color: textColor, marginLeft: 8 }]}>
            {item.fullName}
          </Animated.Text>
        )}
      </Animated.View>
